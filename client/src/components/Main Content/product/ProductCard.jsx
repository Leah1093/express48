...
const ProductCard = React.memo(({ product, favorites }) => {
  ...
  const handleAddToCart = (event) => {
    event.stopPropagation(); // מונע מהכפתור לגרום לניווט
    if (user) {
  ...
  return (
    <div onClick={() => navigate(`/products/${product.storeId.slug}/${product.slug}`)} className="group relative bg-white rounded-xl shadow-md p-4 text-center w-[250px]">
        {/* תגית NEW */}
