    ...
    <div className="w-full h-full flex flex-col gap-6">
      ...
      <div className="flex md:hidden gap-4 w-full overflow-x-auto snap-x snap-mandatory scroll-smooth scrollbar-hide">
        {product.images?.map((img, i) => (
          <div
            key={i}
            className="flex-shrink-0 w-full h-[400px] snap-center rounded-[12px] border border-[#EDEDED] overflow-hidden"
          >
            <img
              src={img}
              alt={`mobile-${i}`}
        ...
        <div className="flex gap-4 w-full overflow-x-auto scrollbar-hide">
          {product.images?.map((img, i) => (
            <div
              ...
              className={`w-[100px] aspect-square flex-shrink-0 cursor-pointer rounded-[12px] border overflow-hidden transition
        ${selectedImage === img
