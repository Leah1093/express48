...
//   );
// }
// src/features/store/StoreSettingsForm.jsx
// src/features/store/StoreSettingsForm.jsx
import { useEffect, useRef, useState } from "react";
import { useForm, Controller } from "react-hook-form";
import { z } from "zod";
...
export default function StoreSettingsForm({
  ...
  // RTK Query: העלאת מדיה
  const [uploadStoreMedia, { isLoading: isUploading }] = useUploadStoreMediaMutation();
  ...
  const slugFieldProps = {
    ...
    className: `mt-1 block w-full rounded-md border-gray-300 shadow-sm ${
      !canChangeSlug ? "bg-slate-50 cursor-not-allowed" : ""
    }`,
    placeholder: "למשל: my-store-name",
  };
    ...
    <form onSubmit={handleSubmit((data) => onSubmit?.(data), onError)} className="space-y-6" noValidate>
          ...
          <div className="grid gap-3">
            <div>
              ...
              <input
                {...{
                  ...register("slug"),
                  readOnly: !(isDraft && !initial?.slugChanged),
                  className: `mt-1 block w-full rounded-md border-gray-300 shadow-sm ${
                    !(isDraft && !initial?.slugChanged) ? "bg-slate-50 cursor-not-allowed" : ""
                  }`,
                  placeholder: "למשל: my-store-name",
                }}
              />
              {!(isDraft && !initial?.slugChanged) ? (
      ...
      <div className="flex flex-col sm:flex-row gap-3 justify-end">
        ...
        <button
          ...
          className="bg-teal-700 text-white px-4 py-2 rounded hover:bg-teal-600 disabled:opacity-60"
          disabled={isUploading}
        >
          {isUploading ? "מעלה…" : "העלאת מדיה"}
        </button>
      </div>
    </form>
  );
