      ...
      <div className="flex flex-col gap-6 w-full">
        <div className="space-y-6">
          ...
          {overview?.text && (
            <div
              className="text-[#141414] text-right leading-relaxed space-y-2"
              dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(overview.text) }}
            />
          )}
            ...
            <div className="flex flex-col items-end gap-4">
              {overview.images.map((img, i) => (
                <img
                  ...
                  className="rounded-lg object-contain ml-auto"
                  style={{ maxWidth: "400px", height: "auto" }}
                  onError={(e) => console.warn("Image failed to load:", img)}
                />
              ))}
            </div>
          )}
            ...
            <div className="flex flex-col items-end">
              {overview.videos.map((vid, i) => (
                <video
                  ...
                  className="rounded-lg ml-auto"
                  style={{ maxWidth: "600px", height: "auto" }}
                  onError={() => console.warn("Video failed to load:", vid)}
                >
                  <source src={vid} type="video/mp4" />
                  הדפדפן שלך לא תומך בניגון וידאו
                </video>
