...
// src/features/store/StoreMediaForm.jsx
import { useState } from "react";
import { useForm, Controller } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import { storeMediaSchema } from "../../validations/storeSchemas";
...
export default function StoreMediaForm({
  initial,
  onUploaded, // יקבל את ה-store המעודכן מהשרת אחרי העלאה
}) {
  ...
  // RTK Query mutation
  const [uploadStoreMedia, { isLoading }] = useUploadStoreMediaMutation();
  ...
  const submitMedia = async (data) => {
    ...
    try {
      ...
      console.error(e);
      alert(e?.data?.message || "העלאת מדיה נכשלה");
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit(submitMedia)} className="space-y-6" noValidate>
      <Section title="לוגו ובאנרים">
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              {...register("bannerTypeStore")}
              onChange={(e) => {
                register("bannerTypeStore").onChange(e);
            ...
            <Controller
              ...
              render={({ field }) => (
                <UploadMedia
                  ...
                  value={field.value}
                  onChange={field.onChange}
                  kinds={bannerKinds.store}
                  accept="auto"
                  hideKindSelector={bannerKinds.store.length === 1}
                  disabled={isLoading}
                />
              )}
            />
            <Controller
              ...
              render={({ field }) => (
                <UploadMedia
                  ...
                  value={field.value}
                  onChange={field.onChange}
                  kinds={bannerKinds.mobile}
                  accept="auto"
                  hideKindSelector={bannerKinds.mobile.length === 1}
                  disabled={isLoading}
                />
              )}
            />
        ...
        <div className="grid md:grid-cols-2 gap-6 mt-4">
          ...
          <select
            ...
            {...register("bannerTypeList")}
            onChange={(e) => {
              register("bannerTypeList").onChange(e);
          ...
          <Controller
            ...
            render={({ field }) => (
              <UploadMedia
                ...
                value={field.value}
                onChange={field.onChange}
                kinds={bannerKinds.list}
                accept="auto"
                hideKindSelector={bannerKinds.list.length === 1}
                disabled={isLoading}
              />
            )}
          />
      ...
      <div className="flex justify-end gap-3">
        <button
          type="submit"
          className="bg-teל-700 text-white px-4 py-2 rounded hover:bg-teal-600 disabled:opacity-60"
          disabled={isLoading}
        >
          {isLoading ? "מעלה…" : "העלאת מדיה"}
        </button>
      </div>
    </form>
  );
