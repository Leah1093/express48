...
import {
  ...
  updateStatusSchema,
  idParamSchema,
} from "../validations/sellerProductsSchemas.js";
...
function validateSchedulingFields({ scheduledAt, visibleUntil }) {
  // עוזר: בדיקות תאריכים מעבר לוולידציית Zod
...
function stripSellerControlledFields(body) {
  // עוזר: מסנן שדות שסלר לא יכול לשלוח
  const {
    sellerId,
    storeId,
    status,
...
export default class SellerProductsController {
  ...
  async getOne(req, res, next) {
    try {
      ...
      const { product, etag, notFound, forbidden } = await service.getOne({
        id,
        sellerId,
        storeId,
        role,
    ...
    try {
      ...
      if (data?.discount) {
        ...
        if (data.discount.startsAt)
          data.discount.startsAt = new Date(data.discount.startsAt) || null;
        if (data.discount.expiresAt)
          data.discount.expiresAt = new Date(data.discount.expiresAt) || null;
      }
      if (
        data?.visibility &&
        !["public", "private", "restricted"].includes(data.visibility)
      ) {
      ...
      if (preconditionFailed)
        return res.status(412).json({ error: "PreconditionFailed" });
  ...
  async create(req, res, next) {
    ...
    try {
      ...
      if (role === "seller") {
        if (!user.sellerId || !user.storeId) {
          const e = new Error("חסר שיוך מוכר/חנות למשתמש. פנה לתמיכה.");
          e.status = 403;
          throw e;
        }

        const cleaned = stripSellerControlledFields(req.body);
        const data = {
          ...cleaned,
        ...
        validateSchedulingFields(data);
        const product = await service.createProduct({
          data,
          actor: { id: user.sub || user._id, role },
        });
      ...
      if (role === "admin") {
        if (!req.body?.sellerId || !req.body?.storeId) {
